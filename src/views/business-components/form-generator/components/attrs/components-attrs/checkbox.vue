...
const changeButtonType = (val: any) => {
  ...
  generatorStore.setCurrentComponent(current.value)
}
...
<template>
  <el-form label-width="90px">
    ...
    <el-form-item label="选项配置">
      ...
      >
        ...
        <el-button @click="del(item, index)" type="danger" circle>
          <template #icon>
  ...
  <el-dialog v-model="editVisible" title="编辑数据源" width="30%">
    <el-form v-if="current" :model="current.children[currentIndex]" label-width="100px">
      ...
      <el-form-item
        label="选项尺寸"
        v-if="current.children[currentIndex].type === 'checkbox-button'"
      >
        <el-select
          v-model="current.children[currentIndex].attrs.size"
          placeholder="请选择输入框尺寸"
        >
